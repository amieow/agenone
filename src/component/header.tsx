...
export default memo(function Header(): JSX.Element {
    ...
    return (
        <header>
            ...
            <nav className="flex p-5 md:px-10 justify-between transition-all md:sticky md:top-0 md:w-full md:justify-around 975:justify-between z-10 ">
                ...
                <div
                    className={`${open &&"fixed flex z-50 top-[100px] w-[75%] max-md:bg-[#0c111fb2] left-0 "} ${mobileOpen && " left-[-70%]"} transition-all duration-200 ease-out md:static md:w-fit `}>
                    <ul className={`${open? " transition-all w-full ": "hidden static right-[-70%]"} py-3 justify-around md:static max-md:divide-y-2  mx-auto md:w-[502px] md:flex `}>
                        {Menu.map((item, index) => {
                            return (
                                <li key={index} className="h-fit my-auto text-center w-auto">
                                    <a className="test text-lg font-medium text-gray-300 relative w-full my-auto hover:text-blue-400"href={item.link}>
                                        {item.title}
